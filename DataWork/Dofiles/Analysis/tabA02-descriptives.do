...
********************************************************************************
		...
		CREATES:	   	Table A2: Descriptive Statistics – Main Outcome Variables
						"${out_tab}/tabA02-descriptives.tex"
...
* ---------------------------------------------------------------------------- */
	...
	* Load data
	...
	preserve
		...
		collapse (sum) bp_v0_target 								///
					   bp_v?_boxvalue_cumul			 			 	///
				(mean) ad_d_equipment ad_success ad_proirri_cost	///
					   el_penalty_mean								///
				  , by(${assocIdVars})
				
		local     	   assoc_bp_vars bp_v0_target   bp_v?_boxvalue_cumul
		local	  	   assoc_ad_vars ad_d_equipment ad_success 		ad_proirri_cost el_penalty_mean
		
		foreach varGroup in assoc_bp assoc_ad {
	...
	replace v1 = "Cumulative scheme savings -- First quarter"      		in 2
	replace v1 = "Cumulative scheme savings -- Second quarter"     		in 3
	replace v1 = "Cumulative scheme savings -- Third quarter"      		in 4
	replace v1 = "Cumulative scheme savings -- Forth quarter"      		in 5
	...
	* Remove lines from `dataout` export
	...
	replace	v1 = "\addlinespace[0.75em] Share of reported penalty outcomes"  				   in 4
	
	gen 	v2 = string(assoc_ad1, "%9.3f")
	gen 	v3 = string(assoc_ad2, "%9.3f")
	
	gen 	v4 = string(assoc_ad3, "%9.0f")
	replace v4 = subinstr(v4,".000","",.)
	gen 	v5 = string(assoc_ad4, "%9.0f")
	replace v5 = subinstr(v5,".000","",.)
	...
	replace v7 = "Household endline survey" in 4
...
* ---------------------------------------------------------------------------- *
	...
	local     hh_bp_vars bp_v?_d_saved_cumul bp_v?_boxvalue_cumul
	...
	* Household level. Box pick-up data	
	...
	gen     v1 = "\addlinespace[0.75em] Saved by the first quarter"    		 in 1
	replace v1 = "Saved by the second quarter"      				   		 in 2
	replace v1 = "Saved by the third quarter"       				   		 in 3
	replace v1 = "Saved by the fouth quarter"       				   		 in 4
	replace v1 = "\addlinespace[0.75em] Cumulative savings -- First quarter" in 5
	replace v1 = "Cumulative savings -- Second quarter" 					 in 6
	replace v1 = "Cumulative savings -- Third quarter" 						 in 7
	replace v1 = "Cumulative savings -- Fourth quarter" 				     in 8
	...
	* Remove lines from `dataout` export
	...
	replace v1 = "\addlinespace[0.75em] Costs of workers" in 11
	replace v1 = "Costs of seeds"						  in 12
	replace v1 = "Costs of equipment rent" 	  		  	  in 13
	replace v1 = "Costs of plot rent" 	  		  		  in 14
	replace v1 = "Costs of livestock" 	  		  		  in 15
	...
	forv varNum = 2/5 {
		...
		replace v`varNum' = string(hh_el`=`varNum'-1', "%9.0f") if regexm(v1, "Costs of")
	}
...
* ---------------------------------------------------------------------------- *
	...
	* Append estimations in unique LaTeX file 								
	foreach varGroup in assoc_bp assoc_ad hh_bp hh_el {
	...
	* Add link to the file ([filefilter] does not provide it automatically)
	di as text `"Open final file in LaTeX here: {browse "${out_tab}/tabA02-descriptives.tex":${out_tab}/tabA02-descriptives.tex}"'
